...
3c move; **pending Mitra's independent verification**) → 4 `chat_block_reasons`/`account_suspensions`/`keyword_blocklist`/
`flag_patterns`/`conversation_reviews` with the message-trigger repoint → 5 retire `profiles` as admin authority.

...
- **Q-4 — DECIDED and applied in Phase 3c (`20260916090000`): admin-only.** `admin_ad_review_log_list` no longer admits the owning vendor (a vendor now gets 42501). Harness `05` confirms this is the only changed cell of 42. Original note:
- **Q-15 — CLOSED in Phase 3c:** `guard_ad_deletion()` is SECURITY DEFINER (search_path `''`), reads `admin.ad_review_log`, and its caller bypass is `current_setting('role', true) is distinct from 'authenticated'`. A literal SECURITY DEFINER conversion with the old `current_user` line was proven to disable the guard. Harness `06` shows the deletion matrix identical before and after: owner+reviewed 42501, owner+never-reviewed deleted, super_admin/service_role/postgres deleted, cascade into the log intact.
- **Q-13 — PARTLY CLOSED in Phase 3c:** anon/authenticated table grants on `admin_flags` and `ad_review_log` are revoked. The same residual grants remain on the Phase 4 tables (`chat_block_reasons`, `account_suspensions`, `keyword_blocklist`, `flag_patterns`, `conversation_reviews`) and on the wider `public` default ACL.
- (Q-4 original, reopened in Phase 3a): the Phase 3 correctness rule ("reproduce the table's current RLS exactly") overrides the Phase 0 plan to drop `ad_review_log_select`'s owner branch. `admin_ad_review_log_list(p_ad_id)` therefore admits the campaign's owning vendor as well as admins. Nothing in the vendor app calls it. Decide explicitly whether vendors keep read access to their campaign's decision history once the table is in `admin`. If not, narrow the RPC in its own migration and update harness `04`.

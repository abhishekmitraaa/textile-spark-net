...
- **`vendor_profiles.cin` and the aadhaar KYC document are unreachable (2026-09-09).** Both are in `VendorOnboardingPayload` and both are wired into `saveVendorOnboarding()`, but `setCin`/`setAadhaar` have no call site anywhere in `Onboarding.tsx` — the inputs were never built. `cin` can only ever be null and those `vendor_documents` rows can never exist, while step 1's "documents required for registration" dialog still tells vendors to have their Aadhaar and CIN ready. Only PAN and GST are reachable.

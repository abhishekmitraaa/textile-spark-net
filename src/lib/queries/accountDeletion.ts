...
//
// sendDeletionCode() → the `account-deletion` edge function emails a 6-digit
// code to the confirmed address on auth.users → confirmDeletion(code) starts a
// 14-day cooling-off → the daily account-deletion-sweep anonymizes the account.
// cancelDeletion() stops it at any point before the sweep.
//
// Every write is a server function. The client holds SELECT on its own request
// row and nothing else, so a stolen session cannot skip the emailed code or the
// 14 days by writing the row itself. Schema, rules and the reasons below:
// supabase/migrations/20260923115839_account_deletion_requests.sql.
// ─────────────────────────────────────────────────────────────
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<OpenDeletionRequest | null> => {
      const { data, error } = await supabase
        ...
        .select("id, status, requested_at, scheduled_for, code_expires_at")
        .eq("user_id", user!.id)
...
export function deletionCopy(r: DeletionResult): DeletionCopy {
  switch (r.status) {
    case "not_configured":
      return { title: "Account deletion isn't available online yet", description: `Email confirmation isn't set up. Write to ${SUPPORT} and we'll delete it for you.` };
    case "vendor":
    ...
    case "no_email":
      return { title: "We can't confirm this by email", description: `Your account has no confirmed email address. Write to ${SUPPORT}.` };
    case "deleted":
    ...
    case "send_failed":
      return { title: "We couldn't send the email", description: r.message };
    default:
